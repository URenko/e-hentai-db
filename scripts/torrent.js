				...
				const request = https.request({
					...
					res.on('end', () => {
						try {
							...
							const torrentRegex = /name="gtid"\svalue="(\d+?)"[\s\S]*?Posted:<.*?(\d{4}-\d{2}-\d{2} \d{2}:\d{2})<\/[\s\S]*?Size:.*>\s?([\d.KMGTiB ]+)<\/[\s\S]*?Uploader:.*?([\S]+)<\/[\s\S]*?(?:([0-9a-f]{40})\.torrent|(value="Expunged"))[\s\S]*?>(?:\s*?&nbsp;\s*?)?(.*?)(?:<\/a>)?<\/td>\s*?<\/tr>\s*?<\/table>/g;
							let exec;
							let list = [];
